...
public class ProjectionProcessor extends AbstractProcessor {
                ...
                private List<JavaClassSource> createFieldProjectionClasses(String packageName) {
                                ...
                                var potentialNewMethods = rootMethods
                                                                ...
                                                                .filter(m -> ignoreMethods.stream()
                                                                                                .noneMatch(im -> m.getSimpleName()
                                                                                                                                .toString()
                                                                                                                                .contains(im)))
                                                                .toList();

                                var collectionMethodsAndOverrides = findMethodsOfType(potentialNewMethods, "Collection",
                                                                "Collection");
                                ...
                                var numberMethodsAndOverrides = findMethodsOfType(potentialNewMethods, "Number",
                                                                "Number");
                                var stringMethodsAndOverrides = findMethodsOfType(potentialNewMethods,
                                                                "Expression<java.lang.String>",
                                                                "String");
                                var intMethodsAndOverrides = findMethodsOfType(potentialNewMethods,
                                                                "Expression<java.lang.Integer>",
                                                                "Integer");
                                var floatMethodsAndOverrides = findMethodsOfType(potentialNewMethods,
                                                                "Expression<java.lang.Float>",
                                                                "Float");
                                var booleanMethodsAndOverrides = findMethodsOfType(potentialNewMethods,
                                ...
                                var main = fieldProjections(packageName,
                                                                potentialNewMethods.stream().filter(
                                                                                                m -> !excludeFromBaseMethods.contains(
                                                                                                                                m))
                                                                                                .toList());

                                var comparable = fieldComparableProjections(packageName, main,
                                                                comparableMethodsAndOverrides);
                                var collection = fieldCollectionProjections(packageName, main,
                                                                collectionMethodsAndOverrides);
                                var comparableNumberClass = fieldNumberComparableProjections(packageName, comparable,
                                                                numberMethodsAndOverrides);
                                var numberFieldClass = fieldNumberProjections(packageName, main,
                                                                numberMethodsAndOverrides);
                                var stringFieldClass = stringFieldProjection(packageName, main,
                                                                stringMethodsAndOverrides);
                ...
                private List<ExecutableElement> findMethodsOfType(List<ExecutableElement> potentialNewMethods,
                                ...
                                return potentialNewMethods
                                                                .stream()
                                                                .filter(m -> stringMethods.contains(m.getSimpleName()
                                                                                                .toString()))
                                                                .toList();
                }

                private JavaClassSource fieldProjections(String packageName,
                                                List<ExecutableElement> potentialNewMethods) {
                                                                                ...
                                                                                .setBody("""
                                                                                                                ...
                                                                                                                .formatted(m.getSimpleName(),
                                                                                                                                                params.wrapped.stream().collect(Collectors
                                                                                                                                                                                .joining(", "))));
                                                m.getTypeParameters()
                                                                                .forEach(tp -> addTypeVariable(newM,
                                                                                                                tp));

                                                params.wrapper.forEach(
                                                                                p -> newM.addParameter(p.asType()
                                                                                                                .toString(),
                                ...
                                methods.forEach(m -> {
                                                ...
                                                if (!seenParams.contains(m.getSimpleName().toString()
                                                                ...
                                                                seenParams.add(m.getSimpleName().toString()
                                                                                                + params.wrapped.toString());
                                                                var retType = m.getReturnType().toString().replaceFirst(
                                                                                                "\\<[A-Z]\\>", "<N>");
                ...
                private JavaClassSource fieldComparableProjections(String packageName, JavaClassSource parent,
                                ...
                                methods.forEach(m -> {
                                                ...
                                                var retType = m.getReturnType().toString().replaceFirst("\\<[A-Z]\\>",
                                                                                "<Y>");
                                                var newMethod = src.addMethod()
                                                                                .setName(m.getSimpleName().toString())
                                                                                .setReturnType(retType)
                ...
                private ParameterHelper getWrappedCallParameters(ExecutableElement m, String boundType) {
                                ...
                                if (!parameters.isEmpty()) {
                                                ...
                                                if (isReplaceableExpression(firstType, m, boundType)) {
                                                                handlesExpression = 0;
                                                                parameters.remove(0);
                                                } else if (boundType != null && lastType.toString()
                                                                                .matches(replaceableExpressionPattern(
                                                                                                                boundType))) {
                ...
                private boolean isReplaceableExpression(TypeMirror type, ExecutableElement method, String boundType) {
                                ...
                                return typeName.matches(replaceableExpressionPattern(boundType))
                                                                || (typeName.contains("Expression") && !method
                                                                                                .getTypeParameters()
                                                                                                .isEmpty()
                                                                                                && method.getTypeParameters().get(
                                                                                                                                0)
                                                                                                                                .getBounds()
                                                                                                                                .stream()
                                                                                                                                .map(Object::toString)
                                                                                                                                .anyMatch(bound -> bound.contains(
                                                                                                                                                                boundType)));
                }
                ...
                private void addTypeVariable(MethodSource<JavaClassSource> method, TypeParameterElement tp) {
                                ...
                                var bounds = tp.getBounds().stream()
                                                                .filter(bound -> !"java.lang.Object".equals(
                                                                                                bound.toString()))
                                                                .map(Object::toString)
